...
namespace logic {
Score::Score(std::string filePath) : _score_file_path(std::move(filePath)) { loadHighScores(); }
...
void Score::saveHighScores() const {
    std::string content;
    for (auto s = _high_scores.rbegin(),
              end = (_high_scores.size() > 5 ? std::prev(_high_scores.rend()) : _high_scores.rend());
         s != end; ++s) {
...
void Score::updateHighScores() {
    if (_current_score <= *_high_scores.rbegin() && !_high_scores.empty())
        return;
...
const std::set<int>& Score::getHighScores() const { return _high_scores; }
