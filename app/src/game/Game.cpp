...
namespace representation {
    void Game::checkMapConsistent() {
        ...
        const bool inconsistent_lines_lengths = std::ranges::any_of(map, [_first_line_len](const auto &line) {
            return line.size() != _first_line_len;
        });

        if (const bool _correct_ratio = 2.1 <= static_cast<float>(_first_line_len) / map.size() && static_cast<float>(
                                            _first_line_len) / map.size() <= 2.3;
