...
MenuState::MenuState(StateManager& manager, Game& game) : State(manager, game) {
    ...
    titleText.setPosition(size.x / 2.f - titleText.getLocalBounds().width / 2.f, size.y * 0.25f);
    ...
    instructionText.setPosition(size.x / 2.f - instructionText.getLocalBounds().width / 2.f, size.y * 0.55f);
}
