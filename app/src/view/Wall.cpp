...
Wall::Wall(const std::shared_ptr<logic::EntityModel>& model) : EntityView(model, 0, 8, 2) {
    ...
    AnimationManager::setFrames(18, 16, 4, _t_kruispunt_frames);
    ...
    bool most_left_wall = compareFloats(model->getPosition().first, -1.0f);
    bool most_right_wall = compareFloats(model->getPosition().first, 1.0f - _model_width);

    bool top_left = most_left_wall && compareFloats(model->getPosition().second, -1.0f);
    bool top_right = compareFloats(model->getPosition().first, 1.0f - _model_width) &&
                     compareFloats(model->getPosition().second, -1.0f);
    bool bottom_left = compareFloats(model->getPosition().first, -1.0f) &&
                       compareFloats(model->getPosition().second, 1.0f - _model_height);
    bool bottom_right = most_right_wall && compareFloats(model->getPosition().second, 1.0f - _model_height);
...
void Wall::updateWall() {
    switch (std::dynamic_pointer_cast<logic::Wall>(_model.lock())->getType()) {
...
void Wall::render(sf::RenderWindow& window) const { EntityView::render(window); }
} // namespace representation
