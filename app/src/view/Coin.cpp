...
namespace representation {
    Coin::Coin(const std::shared_ptr<logic::Coin> &model) : EntityView(model) {
        ...
        _shape.setOrigin(_shape.getPosition().x/2.0, _shape.getPosition().y/2.0);

        auto [x, y] = _model.lock()->getPosition();
        auto [screen_x, screen_y] = Camera::instance().worldToScreen(x, y);


        _shape.setPosition(Constants::COIN_RADIUS + screen_x + Constants::SPRITE_SIZE / 2.0,
                           Constants::COIN_RADIUS + screen_y + Constants::SPRITE_SIZE / 2.0);
